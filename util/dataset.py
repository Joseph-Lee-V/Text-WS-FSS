...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None, data_set=None,
                 ...
                 transform=None, transform_tri=None, mode='train', ann_type='mask', transform_cls=None,
                 transform_clip=None,
                 transform_tri_cls=None, transform_tri_clip=None, ft_transform=None, ft_aug_size=None,
                 ms_transform=None):
        ...
        self.transform_tri_cls = transform_tri_cls
        self.transform_clip = transform_clip
        self.transform_tri_clip = transform_tri_clip
    ...
    def __getitem__(self, index):
        ...
        support_image_cls_list = [[] for _ in range(self.shot)]
        support_label_cls_list = [[] for _ in range(self.shot)]
        support_image_clip_list = [[] for _ in range(self.shot)]
        support_label_clip_list = [[] for _ in range(self.shot)]
        if self.transform is not None:
            image, label, label_b = self.transform_tri(image, label, label_b)  # transform the triple
            image_cls, label_cls, label_b_cls = self.transform_tri_cls(image, label, label_b)
            image_clip, label_clip, label_b_clip = self.transform_tri_clip(image, label, label_b)
            for k in range(self.shot):
                support_image_list[k], support_label_list[k] = self.transform(support_image_list_ori[k],
                                                                              support_label_list_ori[k])
                support_image_cls_list[k], support_label_cls_list[k] = self.transform_cls(support_image_list[k],
                                                                                          support_label_list[k])
                support_image_clip_list[k], support_label_clip_list[k] = self.transform_clip(support_image_list[k],
                                                                                             support_label_list[k])

        s_xs_cls = support_image_cls_list
        s_ys_cls = support_label_cls_list
        s_x_cls = s_xs_cls[0].unsqueeze(0)
        for i in range(1, self.shot):
            s_x_cls = torch.cat([s_xs_cls[i].unsqueeze(0), s_x_cls], 0)
        s_y_cls = s_ys_cls[0].unsqueeze(0)
        for i in range(1, self.shot):
            s_y_cls = torch.cat([s_ys_cls[i].unsqueeze(0), s_y_cls], 0)

        s_xs_clip = support_image_clip_list
        s_ys_clip = support_label_clip_list
        s_x_clip = s_xs_clip[0].unsqueeze(0)
        for i in range(1, self.shot):
            s_x_clip = torch.cat([s_xs_clip[i].unsqueeze(0), s_x_clip], 0)
        s_y_clip = s_ys_clip[0].unsqueeze(0)
        for i in range(1, self.shot):
            s_y_clip = torch.cat([s_ys_clip[i].unsqueeze(0), s_y_clip], 0)

        # Return
        if self.mode == 'train':
            return image_cls, label_cls, label_b_cls, s_x_cls, s_y_cls, subcls_list, \
                class_chosen, image_clip, label_clip, label_b_clip, s_x_clip, s_y_clip
        elif self.mode == 'val':
            return image_cls, label_cls, label_b_cls, s_x_cls, s_y_cls, subcls_list, raw_label, raw_label_b, \
                class_chosen, image_clip, label_clip, label_b_clip, s_x_clip, s_y_clip
        elif self.mode == 'demo':
            total_image_list = support_image_list_ori.copy()
            total_image_list.append(raw_image)
            return image_cls, label_cls, label_b_cls, s_x_cls, s_y_cls, subcls_list, \
                total_image_list, support_label_list_ori, support_label_list_ori_mask, raw_label, raw_label_b, \
                class_chosen, image_clip, label_clip, label_b_clip, s_x_clip, s_y_clip