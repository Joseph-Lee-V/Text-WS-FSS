...
# CUDA_VISIBLE_DEVICES=6,7 nohup python3 -m torch.distributed.launch --nproc_per_node=2 --master_port=6001 train.py --config=config/pascal/pascal_split0_resnet50.yaml --arch=BAM_volo 2>&1 | tee exp/pascal/BAM_volo/split0/resnet50/result/train_2023_0412_0956.log >> /dev/null 2>&1 &
def get_parser():
    parser = argparse.ArgumentParser(description='PyTorch Few-Shot Semantic Segmentation')
    parser.add_argument('--arch', type=str, default='WS_model')  #
    ...
    parser.add_argument('--clip_pretrain', help='clip_pretrain', default=True)
    parser.add_argument('--zero_shot', help='zero_shot', default=False)
    parser.add_argument('--lseg', help='lseg', default=False)
...
def main():
    ...
    mean_clip = [item * value_scale for item in mean_clip]
    std_clip = [0.26862954, 0.26130258, 0.27577711]
    std_clip = [item * value_scale for item in std_clip]
    ...
    train_transform_cls = transform.Compose([
        transform.ToTensor(),
        transform.Normalize(mean=mean, std=std)])
    train_transform_clip = transform.Compose([
        transform.Resize(224),
        transform.ToTensor(),
        transform.Normalize(mean=mean_clip, std=std_clip)])
    ...
    train_transform_tri_cls = transform_tri.Compose([
        transform_tri.ToTensor(),
        transform_tri.Normalize(mean=mean, std=std)])
    train_transform_tri_clip = transform_tri.Compose([
        transform_tri.Resize(224),
        transform_tri.ToTensor(),
        transform_tri.Normalize(mean=mean_clip, std=std_clip)])
    if args.data_set == 'pascal' or args.data_set == 'coco':
        train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                     ...
                                     transform=train_transform, transform_tri=train_transform_tri,
                                     transform_cls=train_transform_cls, transform_clip=train_transform_clip,
                                     transform_tri_cls=train_transform_tri_cls,
                                     transform_tri_clip=train_transform_tri_clip,
                                     mode='train', data_set=args.data_set, use_split_coco=args.use_split_coco)
    ...
    if args.evaluate:
        if args.resized_val:
            val_transform = transform.Compose([transform.Resize(size=args.val_size)])
            val_transform_cls = transform.Compose([
                transform.ToTensor(),
                transform.Normalize(mean=mean, std=std)])
            val_transform_clip = transform.Compose([
                transform.Resize(size=224),
                transform.ToTensor(),
                transform.Normalize(mean=mean_clip, std=std_clip)])

            val_transform_tri = transform_tri.Compose([transform_tri.Resize(size=args.val_size)])
            val_transform_tri_cls = transform_tri.Compose([
                transform_tri.ToTensor(),
                transform_tri.Normalize(mean=mean, std=std)])
            val_transform_tri_clip = transform_tri.Compose([
                transform_tri.Resize(size=224),
                transform_tri.ToTensor(),
                transform_tri.Normalize(mean=mean_clip, std=std_clip)])
        else:
            val_transform = transform.Compose([transform.Resize(size=args.val_size)])
        ...
        if args.data_set == 'pascal' or args.data_set == 'coco':
            val_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root,
                                       ...
                                       transform=val_transform, transform_tri=val_transform_tri,
                                       transform_cls=val_transform_cls, transform_clip=val_transform_clip,
                                       transform_tri_cls=val_transform_tri_cls,
                                       transform_tri_clip=val_transform_tri_clip,
                                       mode='val', data_set=args.data_set, use_split_coco=args.use_split_coco)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and epoch % 1 == 0:
            ...
            if mIoU > best_miou:
                ...
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
                    # np.savetxt(filename+".txt", np.array([1]), fmt="%d", delimiter="")
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    if args.use_split_coco:
        text = torch.from_numpy(
            np.loadtxt("text/coco_text.txt")).cuda().float()
        if args.lseg:
            text_not = torch.from_numpy(
                np.loadtxt("text/coco_other.txt")).cuda().float()
        else:
            text_not = torch.from_numpy(
                np.loadtxt("text/coco_text_not.txt")).cuda().float()
    else:
        text = torch.from_numpy(
            np.loadtxt("text/pascal_text.txt")).cuda().float()
        if args.lseg:
            text_not = torch.from_numpy(
                np.loadtxt("text/pascal_other.txt")).cuda().float()
        else:
            text_not = torch.from_numpy(
                np.loadtxt("text/pascal_text_not.txt")).cuda().float()
    for i, (input, target, target_b, s_input, s_mask, subcls, class_chosen, input_c, _, _, s_input_c, _) in enumerate(
            train_loader):
        ...
        s_input_c = s_input_c.cuda(non_blocking=True)

        output, main_loss, aux_loss1, aux_loss2 = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b,
                                                        cat_idx=subcls, cls=class_chosen, text=text,
                                                        text_not=text_not, s_x_c=s_input_c, x_c=input_c)
            ...
            if mIoU > best_miou:
                ...
                if args.shot == 1:
                    filename = args.snapshot_path + '/train_epoch_' + str(epoch - 0.5) + '_{:.4f}'.format(
                        best_miou) + '.pth'
                    # filename = args.snapshot_path + '/train' + '_best' + '.pth'
                else:
                    filename = args.snapshot_path + '/train{}_epoch_'.format(args.shot) + str(
                        epoch - 0.5) + '_best' + '.pth'
                    # filename = args.snapshot_path + '/train{}_'.format(args.shot) + '_best' + '.pth'
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
                    torch.save(
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, class_chosen, input_c, _, _,
                s_input_c, _) in enumerate(val_loader):
